...
function detectPhraseBreak(syl, nextSyl) {
    ...
    // Significant rest after syllable (half rest or longer)
    if (syl.restAfter && syl.restDurationQ >= 2) return true;

    // Word ends and significant vocal gap follows
    if (isWordEnd && syl.gapDurationQ >= 4) return true;

    // Very long note (whole note or longer) at word end
    if (isWordEnd && syl.durationQ >= 4) return true;

...
function detectPhraseBreak(syl, nextSyl) {
    ...
    // This takes priority over noBreakAfter because a pass boundary is structural.
    if (syl.sectionEnd) return true;

    // Semicolon always forces a break, even across volta transitions
    var text = syl.text;
    ...
    // Fullwidth comma always breaks (new line, but not new stanza)
    // (semicolon handled above, before noBreakAfter check)
    if (lastChar === "\uFF0C") return true;

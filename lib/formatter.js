    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        if (systemTexts) {
            ...
            for (var stj = 0; stj < systemTexts.length; stj++) {
                ...
                if (isBackwards && stTick < firstLineTick && line.startTick >= firstLineTick) {
                    continue;
                }
                    ...
                    if (isIntroRepeat) {
                        ...
                        if (systemTexts) {
                            for (var ilst = 0; ilst < systemTexts.length; ilst++) {
                                ...
                                // Skip pre-firstLineTick labels (intro) when the
                                // repeat target is at or after the first lyric.
                                if (ilstTick < firstLineTick && nextLine.startTick >= firstLineTick) continue;

...
function formatPerfLines(lines, introChords, homeChord, title, chords, syllables, systemTexts, fullRepeat, repeatStartTick) {
    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        if (systemTexts) {
            ...
            for (var stj = 0; stj < systemTexts.length; stj++) {
                ...
                var _introInRep = repeatStartTick >= 0 && introFirstChordTick >= 0 &&
        ...
        if (line.sectionEnd) {
            ...
            if (chords && l + 1 < lines.length && trailingChords.length === 0) {
                ...
                if (interlude.length > 0) {
                    ...
                    var introInsideRepeat = repeatStartTick >= 0 && introFirstChordTick >= 0 &&
                        introFirstChordTick >= repeatStartTick;
                    var isIntroRepeat = !introInsideRepeat && introChords && introChords.length > 0 &&
                        interlude.length === introChords.length &&
                        interlude.join(",") === introChords.join(",");
                    var introIncludedInRepeat = isRepeatPass && introInsideRepeat;
                    if (isIntroRepeat) {

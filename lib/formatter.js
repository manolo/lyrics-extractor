...
function expandTextForChords(text, placements) {
    ...
    // Calculate actual positions: each chord needs at least (prev.actualPos + prev.chord.length + 1)
    var actual = [];
    for (var i = 0; i < placements.length; i++) {
        var naturalPos = placements[i].pos;
        var minPos = (i > 0) ? actual[i - 1] + placements[i - 1].chord.length + 1 : 0;
        actual.push(Math.max(naturalPos, minPos));
    }

    // Calculate actual positions: each chord needs room for its text + 1 space
    var cumShift = 0;
    var inserts = [];
    for (var j = 0; j < placements.length; j++) {
        var naturalPos = placements[j].pos;
        var minPos = (j > 0) ? (placements[j - 1].pos + cumShift + inserts.reduce(function(s, ins) {
            return ins.pos <= placements[j - 1].pos ? s + ins.extra : s; }, 0)
            + placements[j - 1].chord.length + 1) : 0;
        // Calculate needed by comparing actual desired pos vs what we have
        var shiftedPos = naturalPos + cumShift;
        var needed = 0;
        if (shiftedPos < minPos) {
            needed = minPos - shiftedPos;
        }
        if (needed > 0) {
            // Find word boundary: insert at nearest space at or before naturalPos
            var insertAt = naturalPos;
            if (insertAt > 0 && insertAt < text.length && text.charAt(insertAt) !== ' ') {
                var bp = insertAt;
    ...
    // Build chord line: position each chord at its natural position + cumulative shifts up to that point
    var chordLine = "";
    for (var m = 0; m < placements.length; m++) {
        var cShift = 0;
        for (var ins = 0; ins < inserts.length; ins++) {
            if (inserts[ins].pos <= placements[m].pos) cShift += inserts[ins].extra;
        }
        var chordPos = placements[m].pos + cShift;
        // Ensure minimum spacing
        var minChordPos = (m > 0) ? chordLine.length + 1 : 0;
        if (chordPos < minChordPos) chordPos = minChordPos;
        while (chordLine.length < chordPos) chordLine += " ";
...
function formatLines(lines, chords, lastOutputChord, chordToTick) {
    var output = "";
    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        for (var ci = 0; ci < chords.length; ci++) {
            ...
            if (chords[ci].tick <= line.endTick) {
                var targetPos = findPosForTick(line.sylMap, chords[ci].tick);
                placements.push({ pos: targetPos, chord: chords[ci].chord });
            } else {
                // Trailing chord: place after text
                placements.push({ pos: trailPos2, chord: chords[ci].chord });
                trailPos2 += chords[ci].chord.length + 1;
            }
...
function formatPerfLines(lines, introChords, homeChord, title, chords, syllables, systemTexts, fullRepeat) {
    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        for (var si = 0; si < line.sylMap.length; si++) {
            ...
            if (sm.chord && sm.chord !== lastChord) {
                ...
                if (chords && si > 0) {
                    ...
                    // Only expand for significant gaps between syllables (> 2 beats)
                    if (sylGap > 960) for (var ci = 0; ci < chords.length; ci++) {
        ...
        var result = expandTextForChords(line.text, placements);
        // DEBUG
        if (line.text && line.text.indexOf('espuma') >= 0) {
            try { process.stderr.write('DBG FINAL: ' + JSON.stringify(placements) + '\nDBG chord: [' + result.chordLine + ']\nDBG text:  [' + result.text + ']\n'); } catch(e) {}
        }
        if (!line.abbreviated) {
            output += (result.chordLine || "") + "\n";
        }
        ...
        if (line.sectionEnd) {
            ...
            if (chords && l + 1 < lines.length && trailingChords.length === 0) {
                ...
                if (nextLine.startTick < line.endTick) {
                    // Backwards tick (repeat pass): interlude is from start of score to next line
                    interlude = getInterludeChords(chords, -1, nextLine.startTick, null);
                } else {

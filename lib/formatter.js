...
function formatLines(lines, chords, lastOutputChord, chordToTick, systemTexts) {
    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        // Append trailing chords to the chord line if they fit in the line width;
        // otherwise emit them on a separate instrumental line below the text.
        var trailingFitsFL = false;
        var trailingWidthFL = 0;
        if (trailingChords.length > 0) {
            for (var ttcFL = 0; ttcFL < trailingChords.length; ttcFL++) {
                trailingWidthFL += trailingChords[ttcFL].length + (ttcFL > 0 ? 1 : 0);
            }
            trailingFitsFL = (line.text.length + 2 + trailingWidthFL) <= 70;
        }
        if (trailingChords.length > 0 && trailingFitsFL) {
        ...
        // Trailing chords didn't fit: emit as separate instrumental line below
        if (trailingChords.length > 0 && !trailingFitsFL) {
    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        if (systemTexts) {
            ...
            for (var stj = 0; stj < systemTexts.length; stj++) {
                ...
                // On backwards tick, skip system texts that are far before the
                // repeat range (e.g. "Alborada" at tick 1920 when repeat starts at 61920).
                // A system text is "far before" if it's below the backwards target minus
                // a reasonable intro range (the intro chords area before the first lyric).
                if (isBackwards && stTick < line.startTick && introFirstChordTick >= 0 && stTick < introFirstChordTick) {
                    // This is a pre-intro label, already handled by the intro section above
                    continue;
                }
        ...
        // Compute width if we appended trailing chords to the chord line.
        // If the result fits in the line width budget, append; otherwise emit
        // the trailing chords on a separate instrumental line below.
        var trailingFits = false;
        var trailingWidth = 0;
        if (trailingChords.length > 0) {
            for (var ttc = 0; ttc < trailingChords.length; ttc++) {
                trailingWidth += trailingChords[ttc].length + (ttc > 0 ? 1 : 0);
            }
            trailingFits = (line.text.length + 2 + trailingWidth) <= 70;
        }

        if (trailingChords.length > 0 && trailingFits) {
        ...
        // Trailing chords don't fit on the chord line: emit as separate instrumental line below
        if (trailingChords.length > 0 && !trailingFits) {
            output += wrapChordLine(trailingChords.join("  "), 70) + "\n";
            lastChord = trailingChords[trailingChords.length - 1];
            if (chords) {

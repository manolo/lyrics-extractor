...
// Trailing chords found in the gap after a line that ends a section: this many or
// more count as a chords-only passage, which is left to the chord-only line printed
// under the section label of that gap instead of trailing the previous lyric line.
var TRAILING_CHORD_THRESHOLD = 4;

// Width the chord line has to fit for the PDF. A trailing run that would push it
// past this gets a line of its own instead of hanging after the lyric.
var MAX_CHORD_LINE_WIDTH = 70;
...
// Supports two modes:
//   "Verse #"              -> auto-increment: VERSE 1, VERSE 2, ...
//   "Verse 1:2:1"          -> explicit sequence: VERSE 1, VERSE 2, VERSE 1, then VERSE
//   "Soloist ann:bob"        -> explicit sequence: SOLOIST ANN, SOLOIST BOB, then SOLOIST
//   "Verse 1-2-1"          -> explicit sequence with dash separator
...
function renderLabel(text, labelCounters) {
    ...
    if (lastSpace >= 0) {
        ...
        if (isColonSeq || isDashSeq) {
            var items = (isColonSeq ? tail.split(":") : dashParts).filter(function(v) { return v.trim() !== ""; });
            ...
            if (seq.idx < seq.items.length) {
                ...
                // Sequence exhausted: return null to suppress label emission
                return null;
            }
            return upper;
        }
    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        if (systemTexts) {
            while (lastSysTextIdx < systemTexts.length && systemTexts[lastSysTextIdx].tick <= line.startTick) {
                var flRendered = renderLabel(systemTexts[lastSysTextIdx].text, flLabelCounters);
                if (flRendered !== null) output += "\n- " + flRendered + " -\n";
...
// introEntries: the intro in the order it is played, each chord with the tick it was written at.
// Optional: without it the intro is still printed whole, and only a split at a label falls back to
// the written order.
function formatPerfLines(lines, introChords, homeChord, title, chords, syllables, systemTexts, fullRepeat, repeatStartTick, repeats, introEntries) {
    // Abbreviate repeated stanzas (e.g. chorus appearing multiple times)
    ...
    var lastRenderedLabel = {}; // raw text -> last rendered label (for re-emission without advancing sequence)
    ...
    if (systemTexts) {
        for (var sti = 0; sti < systemTexts.length; sti++) {
            if (systemTexts[sti].tick < firstLineTick && introFirstChordTick >= 0 && systemTexts[sti].tick <= introFirstChordTick) {
                ...
                if (introLabel !== null) output += "- " + introLabel + " -\n";
    ...
    if (introChords && introChords.length > 0) {
        ...
        if (introSplitTicks.length === 0) {
            ...
            //
            // introEntries is the intro in the order it is played, each chord with the tick it was
            // written at, so a segment is the run of played chords whose written position falls
            // between two labels. Without it there is nothing to cut but the written order, and a
            // repeat printed that way says the impossible: after the last bar of the repeat comes
            // the bar after it, when what is played next is the first bar again.
            var haveEntries = introEntries && introEntries.length > 0;
            ...
            for (var sp = 0; sp <= introSplitTicks.length; sp++) {
                ...
                var segChords = [];
                if (haveEntries) {
                    for (var se = 0; se < introEntries.length; se++) {
                        var entry = introEntries[se];
                        if (entry.tick < splitFrom || entry.tick >= splitTo) continue;
                        if (entry.chord !== splitLastChord) {
                            segChords.push(entry.chord);
                            splitLastChord = entry.chord;
                        }
                    }
                } else if (chords) {
                ...
                output += "\n";
                // The next segment starts where this one ended, otherwise every segment after
                // the first repeats the whole intro
                splitFrom = splitTo;
                // Emit the label for the next segment
                if (sp < introSplitTicks.length) {
                    var spLabel = renderLabel(introSplitTicks[sp].text, labelCounters);
                    if (spLabel !== null) output += "- " + spLabel + " -\n";
                }
            }
        }
            ...
            for (var sti2c = 0; sti2c < systemTexts.length; sti2c++) {
                ...
                if (st2cTick > introFirstChordTick && st2cTick < firstLineTick) {
                    ...
                    if (!emittedSysTexts[st2cKey]) {
                        var st2cLabel = renderLabel(systemTexts[sti2c].text, labelCounters);
                        if (st2cLabel !== null) {
                            output += "\n- " + st2cLabel + " -\n";
                        }
        ...
        if (systemTexts) {
            for (var sti2b = 0; sti2b < systemTexts.length; sti2b++) {
                if (systemTexts[sti2b].tick < firstLineTick && (introFirstChordTick < 0 || systemTexts[sti2b].tick > introFirstChordTick)) {
                    var sti2bLabel = renderLabel(systemTexts[sti2b].text, labelCounters);
                    if (sti2bLabel !== null) output += "\n- " + sti2bLabel + " -\n";
    ...
    for (var l = 0; l < lines.length; l++) {
        var line = lines[l];
        if (!line.text && !line.abbreviated) continue;
        ...
        if (systemTexts) {
            ...
            if (isBackwards) {
                ...
                if (l > 0 && lines[l - 1].segmentBoundary) {
                    // D.S./D.C. jump: allow labels from the section the jump lands in, which
                    // is the last one at or before the target, and everything after it.
                    // Allowing them from the top of the score brought back the headings of
                    // every section before the segno, which this pass never plays; allowing
                    // them only from the target itself dropped the heading of the section it
                    // lands in, when its label sits a little earlier (a pickup measure).
                    prevStartTick = -1;
                    if (line.replayFromTick !== undefined && line.replayFromTick !== null) {
                        var landedLabelTick = -1;
                        for (var rft = 0; rft < systemTexts.length; rft++) {
                            if (systemTexts[rft].tick <= line.replayFromTick &&
                                systemTexts[rft].tick > landedLabelTick) {
                                landedLabelTick = systemTexts[rft].tick;
                            }
                        }
                        prevStartTick = (landedLabelTick >= 0) ? landedLabelTick - 1 : line.replayFromTick - 1;
                    }
                    emittedSysTexts = {};
                    lastRenderedLabel = {}; // new replay group: advance labels afresh
                } else {
            ...
            for (var stj = 0; stj < systemTexts.length; stj++) {
                ...
                if (introFirstChordTick >= 0 && stTick <= introFirstChordTick) {
                    ...
                    // Labels after the last intro chord (e.g. Soloist) label the next section.
                ...
                // labels were emitted (e.g. Verse + Chorus within same repeat),
                ...
                if (stTick > prevStartTick && stTick <= line.startTick) {
                    ...
                    var renderedLabel;
                    var rawText = systemTexts[stj].text;
                    // For sequence labels (e.g. "Verse 1::2::3::"), re-emitting within
                    // the same replay group should reuse the last rendered label without
                    // advancing the counter. This keeps both repeat passes of a D.S. replay
                    // under the same label (e.g. VERSE 2). For # labels, always advance.
                    var isSeqLabel = rawText.indexOf(":") >= 0;
                    var inJumpReplay = line._jumpReplay || false;
                    if (isBackwards && emittedSysTexts[stKey] && isSeqLabel) {
                        if (inJumpReplay && lastRenderedLabel[rawText]) {
                            // Within a D.S. replay: reuse last label (both repeat passes
                            // of the replay share the same label, e.g. VERSE 2).
                            var _upper2 = rawText.toUpperCase();
                            var _ls2 = _upper2.lastIndexOf(" ");
                            var _seqKey2 = (_ls2 >= 0) ? "_seq_" + _upper2.substring(0, _ls2).trim() : null;
                            var _seq2 = _seqKey2 ? labelCounters[_seqKey2] : null;
                            if (_seq2 && _seq2.idx >= _seq2.items.length) continue; // exhausted
                            renderedLabel = lastRenderedLabel[rawText];
                        } else if (!inJumpReplay) {
                            // Normal repeat: advance the sequence (each pass gets next label).
                            renderedLabel = renderLabel(rawText, labelCounters);
                            if (renderedLabel === null) continue;
                            lastRenderedLabel[rawText] = renderedLabel;
                        } else {
                            continue; // inJumpReplay but no lastRenderedLabel yet
                        }
                    } else {
                        renderedLabel = renderLabel(rawText, labelCounters);
                        if (renderedLabel === null) continue; // sequence exhausted, suppress
                        lastRenderedLabel[rawText] = renderedLabel;
                    }
                    output += "\n- " + renderedLabel + " -\n";
                    hasLabelBefore = true;
                    lastLabelTick = stTick;
                    emittedSysTexts[stKey] = true;
                    hadSectionBarSinceLabel = false;
                    // Emit chord-only line for labeled instrumental sections
                    // (e.g. MUSIC with interlude chords between VERSE and CHORUS).
                    // Check chords in [stTick, nextLabelOrLineStart). Only emit when
                    // the gap is substantial (> 4 beats) and there are 2+ distinct chords.
                    // In compact mode (fullRepeat=false), deduplicate across repeat passes.
                    {
                        var _nlTick = line.startTick;
                        for (var _nlj = stj + 1; _nlj < systemTexts.length; _nlj++) {
                            if (systemTexts[_nlj].tick > stTick && systemTexts[_nlj].tick <= line.startTick) {
                                _nlTick = systemTexts[_nlj].tick;
                                break;
                            }
                        }
                        if (_nlTick - stTick > 1920 && chords && stTick >= firstLineTick && (fullRepeat || !emittedLabeledChords[stKey])) {
                            var _ilChords = [];
                            var _ilLastChord = null;
                            for (var _ilc = 0; _ilc < chords.length; _ilc++) {
                                if (chords[_ilc].tick < stTick) continue;
                                if (chords[_ilc].tick >= _nlTick) break;
                                if (chords[_ilc].chord !== _ilLastChord) {
                                    _ilChords.push(chords[_ilc].chord);
                                    _ilLastChord = chords[_ilc].chord;
                                }
                            }
                            if (_ilChords.length >= 2) {
                                output += wrapChordLine(_ilChords.join("  "), 70) + "\n";
                                lastChord = _ilLastChord;
                                emittedLabeledChords[stKey] = true;
                            }
                        }
                    }
                }
        ...
        if (line.abbreviated) {
            if (hasLabelBefore) {
                ...
                // Reset to homeChord (not null) so the home chord suppression still
                // works at the start of the next non-abbreviated section.
                lastChord = homeChord || null;
                output += "\n";
                continue;
            }
        ...
        // Pickup zone: show chords that fall between the section label tick and the
        // first lyric syllable (anacrusis/pickup measure). These chords are placed at
        // position 0 of the chord line before the sylMap chords.
        var placements = [];
        if (hasLabelBefore && chords && lastLabelTick >= 0 && lastLabelTick < line.startTick) {
            var puLastChord = null;
            for (var pu = 0; pu < chords.length; pu++) {
                if (chords[pu].tick < lastLabelTick) continue;
                if (chords[pu].tick >= line.startTick) break;
                if (chords[pu].chord !== puLastChord) {
                    placements.push({ pos: 0, chord: chords[pu].chord });
                    puLastChord = chords[pu].chord;
                }
            }
            if (placements.length > 0) lastChord = puLastChord;
        }
        for (var si = 0; si < line.sylMap.length; si++) {
            ...
            if (sm.chord && sm.chord !== lastChord) {
                ...
                // Exception: when a section label was just emitted and the home chord is a
                // dominant-seventh chord (ends with "7"), always show it. A dominant seventh
                // opening chord is musically significant (transient, resolves to another),
                // so suppressing it would hide information the player needs.
                var homeChordIsDomSeventh = hasLabelBefore && homeChord &&
                    homeChord.charAt(homeChord.length - 1) === '7';
                if (!homeChordIsDomSeventh && placements.length === 0 && sm.pos <= 2 && homeChord && sm.chord === homeChord && lastChord !== null && lastChord !== homeChord) {
                    lastChord = sm.chord;
                    continue;
                }
                var chordPos = sm.pos;
            ...
            // A syllable carries one chord and findChordAtTick gives that slot to the harmony,
            // so an annotation sharing the tick would have nowhere to go. It is placed beside
            // the chord instead: the chord keeps the syllable, the words follow it, because
            // expandTextForChords pushes whatever comes second to the right.
            // Outside the block above on purpose, so the annotation survives on a syllable
            // whose chord was suppressed as a repeat or as a return to the home chord.
            if (chords) {
                for (var an = 0; an < chords.length; an++) {
                    if (chords[an].tick > sm.tick) break;
                    // Not when the annotation is already the syllable's chord: with no harmony
                    // on that tick, findChordAtTick hands the slot to the annotation and it has
                    // been printed by the block above. Placing it again prints it twice.
                    if (chords[an].tick === sm.tick && chords[an].isText && chords[an].chord !== sm.chord) {
                        placements.push({ pos: sm.pos, chord: chords[an].chord });
                    }
                }
            }
        }
        ...
        // Append trailing chords to the chord line above the lyric (rule 3.5).
        // But when there are many trailing chords AND a section break follows,
        // leave them for the interlude handler (they belong between stanzas,
        // not crammed onto the current line).
        // When many trailing chords span a labeled instrumental section
        // (system text between this line and next), defer to interlude handler.
        var trailingHasLabel = false;
        var trailingLabelTick = -1;
        if (trailingChords.length >= TRAILING_CHORD_THRESHOLD && line.sectionEnd && systemTexts) {
            var trailNextStart = (l + 1 < lines.length) ? lines[l + 1].startTick : -1;
            for (var thl = 0; thl < systemTexts.length; thl++) {
                var thlTick = systemTexts[thl].tick;
                if (thlTick >= line.endTick && (trailNextStart < 0 || thlTick < trailNextStart)) {
                    trailingHasLabel = true; trailingLabelTick = thlTick; break;
                }
                // Backwards tick: label in replay range
                if (trailNextStart >= 0 && trailNextStart < line.endTick && thlTick <= line.endTick && thlTick >= trailNextStart) {
                    trailingHasLabel = true; break;
                }
            }
        }
        // With a label in the gap, the chord-only line printed under that label covers
        // the chords from the label onwards, but the ones before it belonged to nobody
        // and were dropped. Emit just that part here.
        var emitChords = trailingChords;
        if (trailingHasLabel) {
            emitChords = (trailingLabelTick > line.endTick)
                ? getInterludeChords(chords, line.endTick, trailingLabelTick, lastChord)
                : [];
        }
        // A run wide enough to push the chord line past the page width goes on a line
        // of its own between the stanzas; short tails stay after the lyric as before.
        var standaloneRun = "";
        if (emitChords.length > 0) {
            var trailingWidth = line.text.length + 2 + emitChords.join("  ").length;
            if (emitChords.length > 1 && trailingWidth > MAX_CHORD_LINE_WIDTH) {
                standaloneRun = wrapChordLine(emitChords.join("  "), MAX_CHORD_LINE_WIDTH);
            } else {
                var trailPos = line.text.length + 2;
                for (var tc = 0; tc < emitChords.length; tc++) {
                    placements.push({ pos: trailPos, chord: emitChords[tc] });
                    trailPos += emitChords[tc].length + 1;
                }
            }
            lastChord = emitChords[emitChords.length - 1];
        ...
        output += result.text + "\n";
        if (standaloneRun) output += standaloneRun + "\n";
        ...
        if (line.sectionEnd) {
            ...
            // A labeled gap is intentionally excluded: the chord-only line under the
            // section label already printed those chords, and routing them through the
            // interlude handler here would emit the next label before them.
            if (chords && l + 1 < lines.length && trailingChords.length === 0) {
                ...
                if (interlude.length > 0) {
                    ...
                    if (isIntroRepeat) {
                        ...
                        if (introInsideRepeat && isRepeatPass && systemTexts) {
                            for (var iil = 0; iil < systemTexts.length; iil++) {
                                if (systemTexts[iil].tick <= introFirstChordTick) {
                                    var iilLabel = renderLabel(systemTexts[iil].text, labelCounters);
                                    if (iilLabel !== null) output += "\n- " + iilLabel + " -\n";
                        ...
                        // "Music" on the second pass of a repeat), emit it first
                        ...
                        if (systemTexts) {
                            for (var ilst = 0; ilst < systemTexts.length; ilst++) {
                                ...
                                var ilstLabel = renderLabel(systemTexts[ilst].text, labelCounters);
                                if (ilstLabel === null) continue;
                                output += "- " + ilstLabel + " -\n";
    ...
    // Skip when endTick < startTick (DC/DS lead-in remapped backwards):
    // codaFromTick would be 0, dumping all chords. The orchestrator handles these.
    if (chords && lines.length > 0) {
        var lastLine = lines[lines.length - 1];
        if (!lastLine.abbreviated && lastLine.endTick >= lastLine.startTick) {
    ...
    // (e.g. "- VERSE -\n\n- VERSE -" from D.S. repeat passes)
    output = output.replace(/(- .+ -)\n\n\1/g, "$1");
    return { text: output };
}

// Detect repeated stanzas (e.g. chorus) and abbreviate subsequent occurrences.
...
// An abbreviated stanza replaces the lines it stands for, so it has to carry the flags
// the caller reads from a line: whether a replay group ends there, and whether it belongs
// to a replay. Losing them left a numbered label reusing its previous value instead of
// advancing on the next pass.
function _groupFlags(lines, idxs) {
    var flags = { segmentBoundary: false, _jumpReplay: false };
    for (var i = 0; i < idxs.length; i++) {
        if (lines[idxs[i]].segmentBoundary) flags.segmentBoundary = true;
        if (lines[idxs[i]]._jumpReplay) flags._jumpReplay = true;
    }
    return flags;
}
...
function abbreviateRepeatedStanzas(lines, syllables, systemTexts) {
    ...
    // same section (e.g. the closing line of a verse) are NOT abbreviated,
    // while repeated sections with their own label (e.g. a second chorus) ARE.
    // Also detect prefix matches: when a stanza starts with the same lines as
    // an earlier one but has extra lines (e.g. volta 2 adds a phrase), abbreviate
    // the common prefix and keep only the extra lines.
    var seen = {};
    var toAbbreviate = {};
    var prefixMatch = {}; // s2 -> { sourceStanza, commonLines }
    var sectionAbbreviated = false;   // a stanza of the section we are in has been abbreviated

    // Build per-line fingerprints for prefix comparison
    var lineFingerprints = [];
    for (var lf = 0; lf < lines.length; lf++) {
        lineFingerprints.push((lines[lf].text || "").trim().toLowerCase().replace(/[.,!?;:]+/g, ""));
    }
    ...
    for (var s2 = 0; s2 < stanzas.length; s2++) {
        ...
        // Check if a label precedes this stanza
        ...
        // A section of more than one stanza carries its label on the first of them, so only that
        // first stanza has a label before it. Once it has been abbreviated, the stanzas that
        // follow it belong to the same abbreviated section and go the same way: a chorus of two
        // stanzas used to lose the first and print the second. A stanza that is not a repeat ends
        // the run, so a section that carries on with something new keeps the protection this rule
        // exists for.
        var canAbbreviate = !systemTexts || !systemTexts.length || hasLabelBefore || isReplay ||
            sectionAbbreviated;

        if (seen[fp] !== undefined) {
            if (canAbbreviate) {
                toAbbreviate[s2] = true;
                sectionAbbreviated = true;
            }
        } else {
            sectionAbbreviated = false;
            // Check for prefix match: does this stanza start with lines
            // identical to a previously seen shorter stanza?
            var foundPrefix = false;
            for (var seenFp in seen) {
                var seenIdx = seen[seenFp];
                var seenLines = stanzas[seenIdx];
                var curLines = stanzas[s2];
                if (curLines.length > seenLines.length) {
                    // Compare line-by-line
                    var allMatch = true;
                    for (var pl = 0; pl < seenLines.length; pl++) {
                        if (lineFingerprints[curLines[pl]] !== lineFingerprints[seenLines[pl]]) {
                            allMatch = false; break;
                        }
                    }
                    if (allMatch && canAbbreviate) {
                        prefixMatch[s2] = { sourceStanza: seenIdx, commonLines: seenLines.length };
                        foundPrefix = true;
                        break;
                    }
                }
    ...
    if (Object.keys(toAbbreviate).length === 0 && Object.keys(prefixMatch).length === 0) return lines;

    // Build new lines array, replacing duplicate stanzas with incipit
    var result = [];
    for (var s3 = 0; s3 < stanzas.length; s3++) {
        if (prefixMatch[s3]) {
            // Prefix match: abbreviate the common lines, keep the extra ones.
            // Emit an abbreviated marker for the common prefix, then the extra lines.
            var pm = prefixMatch[s3];
            var pmFirstIdx = stanzas[s3][0];
            var pmFlags = _groupFlags(lines, stanzas[s3].slice(0, pm.commonLines));
            result.push({
                text: "",
                sylMap: [],
                startTick: lines[pmFirstIdx].startTick,
                endTick: lines[stanzas[s3][pm.commonLines - 1]].endTick,
                sectionEnd: false,
                abbreviated: true,
                segmentBoundary: pmFlags.segmentBoundary,
                _jumpReplay: pmFlags._jumpReplay
            });
            // Emit extra lines (the non-common tail)
            for (var px = pm.commonLines; px < stanzas[s3].length; px++) {
                result.push(lines[stanzas[s3][px]]);
            }
        } else if (toAbbreviate[s3]) {
            ...
            var abFlags = _groupFlags(lines, stanzas[s3]);
            result.push({
                text: abbrevText,
                sylMap: [],
                ...
                sectionEnd: true,
                abbreviated: true,
                segmentBoundary: abFlags.segmentBoundary,
                _jumpReplay: abFlags._jumpReplay
            });
        } else {
...
// The text as a person reads it: no markers, and an annotation in round brackets.
//
// Braces are the form the pipeline carries, because they cannot be confused with anything a
// chord says and so tell the PDF what to colour and ChordPro what to write as [*text]. That
// argument is about programs reading the line. On a page a reader is not going to mistake
// "(muy suave)" for a chord, and brackets are what a musician writes by hand, so this is the
// last step before the words reach somebody: after the PDF and the .cho have been made from
// the braced form, never before.
function forReading(text) {
    return stripChordMarkers(text).replace(/\{([^}\n]*)\}/g, "($1)");
}

// Remove entire chord lines from output (lyrics only mode)
// Also collapses extra alignment spaces in lyric lines
...
if (typeof exports !== "undefined") {
    ...
    exports.stripChordLines = stripChordLines;

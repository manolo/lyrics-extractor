...
function formatPerfLines(lines, introChords, homeChord, title, chords, syllables, systemTexts, fullRepeat) {
    ...
    // Count system texts in the lyric range (for deciding whether to suppress on repeat)
    var sysTextCountInRange = 0;
    if (systemTexts) {
        for (var stc = 0; stc < systemTexts.length; stc++) {
            if (systemTexts[stc].tick >= firstLineTick) sysTextCountInRange++;
        }
    }
    ...
    for (var l = 0; l < lines.length; l++) {
        ...
        if (systemTexts) {
            ...
            for (var stj = 0; stj < systemTexts.length; stj++) {
                ...
                // ONLY if this is the sole label in the section. If there are multiple
                // labels (e.g. Estribillo + Estrofa + Solista), re-emit on each pass.
                // Also allow re-emission for abbreviated stanzas (label replaces text).
                var stKey = stTick + "_" + systemTexts[stj].text;
                if (isBackwards && emittedSysTexts[stKey] && !line.abbreviated && sysTextCountInRange <= 1) continue;
    ...
    // Use lastChordTick (tick of last emitted chord) to avoid re-emitting
    // chords already shown as trailing on the last chord line.
    if (chords && lines.length > 0) {
        var lastLine = lines[lines.length - 1];
        if (!lastLine.abbreviated) {
            var codaChords = getInterludeChords(chords, lastChordTick, -1, lastChord);

...
function _walkUnwind(data) {
            ...
            if (isFinal) {
                ...
                if (!jumpsTaken[jumpKey]) {
                    ...
                    if (jumpToIdx >= 0) {
                        ...
                        _pendingJumpLeadIn = (jumpMeasureEndTick > 0 && jumpMeasureEndTick > jumpEl.tick)
                            ? { fromTick: jumpEl.tick, toTick: jumpMeasureEndTick } : null;
    ...
    return _annotateSegments(rawSegments, data, _pendingJumpLeadIn);
}

// Convert raw walk segments to the format materialize expects.
// Annotates with volta info, break types, etc.
function _annotateSegments(rawSegments, data, jumpLeadIn) {
    var voltas = data.voltas || [];
    var segments = [];
    ...
    for (var i = 0; i < rawSegments.length; i++) {
        ...
        var mainTo = (voltaFrom >= 0 && voltaFrom > raw.fromTick) ? voltaFrom : raw.toTick;
        ...
        segments.push({
            ...
            breakType: breakType,
            segmentBoundary: raw.isJumpReplay && i > 0 && !rawSegments[i-1].isJumpReplay,
            jumpLeadIn: (raw.isJumpReplay && i > 0 && !rawSegments[i-1].isJumpReplay)
                ? jumpLeadIn : null,
...
function materialize(segments, data) {
    ...
    for (var si = 0; si < segments.length; si++) {
        ...
        // Reset verse counters at the start of a D.S./D.C. replay so the
        // repeated sections use the same verse sequence as the original.
        // Also prepend lead-in syllables from the jump measure (D.S. takes
        // effect at the END of its measure, so syllables after the jump tick
        // but in the same measure play before the jump executes).
        if (seg.isJumpReplay && (si === 0 || !segments[si - 1].isJumpReplay)) {
            verseCounter = {};
            if (seg.jumpLeadIn) {
                var leadSyls = filterSylsByRange(syllables, seg.jumpLeadIn.fromTick, seg.jumpLeadIn.toTick);
        ...
        if (seg.breakType === "section" && stream.length > 0 && !stream[stream.length - 1].noBreakAfter && !voltaAtStart) {
            ...
            // Also suppress at D.S./D.C. transitions when text continues
            var isDSTransition = seg.isJumpReplay && (si === 0 || !segments[si - 1].isJumpReplay);
            if ((crossesVolta || isDSTransition) && !endsWithPunctuation) {
        ...
        if (seg.repeatStartTick >= 0) {
            ...
            targetVerse = verses.length > vIdx ? verses[vIdx] : (verses.length > 0 ? verses[0] : 0);
            verseCounter[repKey]++;

...
function materialize(segments, data) {
    ...
    for (var si = 0; si < segments.length; si++) {
        ...
        if (seg.breakType === "section" && stream.length > 0 && !stream[stream.length - 1].noBreakAfter && !voltaAtStart) {
            ...
            if (crossesVolta && !endsWithPunctuation) {
        ...
        if (pendingLeadIn) {
            ...
            for (var li = 0; li < leadVerse0.length; li++) {
                ...
                // Use the D.S. replay start tick so the formatter places the
                // label BEFORE the lead-in (tick ordering for label emission).
        ...
        if (seg.voltaFrom >= 0 && !voltaAtStart) {
            ...
            if (seg.isImplicitV2) {
                ...
                // But preserve the break when text ends with punctuation.
                if (stream.length > 0) {
                    var lastBefore = stream[stream.length - 1];
                    var lbText = lastBefore.text || "";
                    var lbLast = lbText.charAt(lbText.length - 1);
                    if (lbLast !== "." && lbLast !== "!" && lbLast !== "?") lastBefore.noBreakAfter = true;

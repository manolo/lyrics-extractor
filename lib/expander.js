...
function _computeMeasureSize(data) {
    ...
    for (var i = 0; i < markers.length; i++) {
        if (markers[i].tick > 0) ticks.push(markers[i].tick);
    }
    for (var i = 0; i < jumps.length; i++) {
        if (jumps[i].tick + 1 > 0) ticks.push(jumps[i].tick + 1);
    }
    ...
    for (var si = 0; si < segments.length; si++) {
        ...
        if (voltaAtStart) {
            ...
            for (var ev = 0; ev < earlyVoltaFiltered.length; ev++) {
                stream.push(cloneSyl(earlyVoltaFiltered[ev], chords));
            }
        }
            ...
            if (seg.isImplicitV2) {
                ...
                if (voltaFiltered.length > 0 && si + 1 < segments.length) {
                    ...
                    if (nextSeg.repeatStartTick === seg.repeatStartTick && nextSeg.breakType === "section") {
                        ...
                        // Respect semicolon forced breaks: if the last syl ends with ";"
                        // the user explicitly wants a break here, so don't suppress it.
                        var last = stream[stream.length - 1];
                        var lastText = last.text || "";
                        var hasForcedBreak = lastText.charAt(lastText.length - 1) === ";";
                        if (!hasForcedBreak) {

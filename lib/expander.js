            ...
            if (playUntilIdx >= 0 && it === playUntilIdx) {
                ...
                if (isMarkerFinal) {
                    ...
                    if (rs !== null && !_rsIsEmpty(rs)) {
                        ...
                        // Never reach the section the jump is about to continue at: with no
                        // barline between the marker and the end of the score, the extension
                        // swallowed the coda, which the coda segment then printed again.
                        var cappedByMarker = false;
                        var pumMarkers = data.markers || [];
                        for (var pmi = 0; pmi < pumMarkers.length; pmi++) {
                            var pmTick = pumMarkers[pmi].tick;
                            if (pmTick > markerTick && pmTick < extendedTo) {
                                extendedTo = pmTick;
                                cappedByMarker = true;
                            }
                        }
                        // Use barline + 1 to include syllables AT the barline tick
                        // (filterSylsByRange uses exclusive upper bound). A cap set by the
                        // next marker is exclusive already: the first syllable of that
                        // section belongs to the section, not to this segment.
                        if (extendedTo > ticks.toTick) {
                            ticks.toTick = cappedByMarker ? extendedTo : extendedTo + 1;
                        }
                        rawSegments.push({ fromTick: ticks.fromTick, toTick: ticks.toTick,
                                           playbackCount: rs.playbackCount,
                                           repeatStartTick: (startRepeatRef._repeatCount > 1) ? startRepeatRef.measure.tick : -1,
...
function materialize(segments, data) {
    ...
    for (var si = 0; si < segments.length; si++) {
        ...
        if (seg.repeatStartTick >= 0) {
            ...
            // continue after the repeat (e.g. chorus) inherit
        ...
        // Verse 0 "tail" detection (chorus pattern, spec 7.6)
        var allMainVerses = {};
...
// ========================================
// Public entry points for section and navigation structure
// ========================================

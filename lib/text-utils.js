...
// Wrap the text of a staff text, expression or play technique for the chord line.
//
// Two jobs, one pair of characters. It says the token is words rather than a harmony, which no
// spelling rule can decide on its own: "Solo" is written exactly the way "Re" plus a diminished
// mark is. And it holds the words together as a single token, which whitespace cannot do,
// because everything that reads a chord line back splits it on spaces.
//
// Braces rather than brackets: a parenthesis is part of the chord vocabulary, "Mi7(b5)", so a
// hand written optional chord "(Do)" would be indistinguishable from words. No chord notation
// uses a brace, and ChordPro's own {directive} form only ever opens a line.
function braceAnnotation(text) {
...
if (typeof exports !== "undefined") {
    ...
    exports.braceAnnotation = braceAnnotation;

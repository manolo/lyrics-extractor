...
function buildPerformanceStream(syllables, chords, repStruct, verseOffset) {
    ...
    for (var sIdx = 0; sIdx < repStruct.sections.length; sIdx++) {
        ...
        // A simple repeat plays twice; numPasses = min(available verses, 2)
        // unless voltas demand more passes
        var numPasses = Math.min(mainVerses.length, 2);
        if (hasVoltaPair) numPasses = Math.max(numPasses, 2);
        if (voltaVerseCount > numPasses) numPasses = Math.min(voltaVerseCount, 2);
        ...
        for (var pass = 0; pass < numPasses; pass++) {
            ...
            if (hasVoltaPair) {
                ...
                var voltaFiltered = filterSylsByVerse(voltaSyls, mainVerse);
                if (voltaFiltered.length === 0 && pass === 0) {
    ...
    for (var sb = 0; sb < stream.length - 1; sb++) {
        ...
        if (sbNext.tick < sbSyl.tick && !sbSyl.noBreakAfter && sbSyl.durationQ > 1) {

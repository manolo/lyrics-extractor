...
// Mirror of Formatter.wrapChordLine (duplicated for QML import compatibility)
...
function processExtraction(data, mods) {
    ...
    if (verseNums.length === 1) {
        ...
        if (lines0.length > 0) {
            ...
            if (introC1.length > 0) {
                output1v += wrapChordLine(introC1.join("  "), 70) + "\n\n";
            }
        }
        ...
        if (codaChords1v.length > 0) {
            output1v += "\n" + wrapChordLine(codaChords1v.join("  "), 70) + "\n";
        }
    ...
    if (introChordsMV.length > 0) {
        outputMV += wrapChordLine(introChordsMV.join("  "), 70) + "\n\n";
    ...
    if (allLines.length > 0) {
        ...
        if (codaChordsMV.length > 0) {
            outputMV += "\n" + wrapChordLine(codaChordsMV.join("  "), 70) + "\n";
        }
    }
...
function processWithNavigation(data, mods) {
    ...
    for (var pi = 0; pi < plan.length; pi++) {
        ...
        if (overlapCount > 0 && !fullRepeat) {
            ...
            if (!hasNewVerse) continue; // skip replay with no unconsumed new lyrics
        }
        ...
        if (hasRepeatStructure && segSyls.length > 0) {
            ...
            var stream = mods.PerfStream.buildPerformanceStream(segSyls, chords, segRepStruct);
        ...
        if (iSegSyls.length === 0) {
            ...
            if (iChords.length > 0) {
                ...
                if (isRepeatOfIntro && iLabel) {
                    ...
                    if (iLabel) output += "\n- " + iLabel.toUpperCase() + " -\n";
                    output += wrapChordLine(iChords.join("  "), 70) + "\n";
                }
            }
        }
    }

...
function processExtraction(data, mods) {
    ...
    if (barlines.length > 0) {
        ...
        // Suppress sectionBar at repeat barlines when the lyrics continue
        // (no final punctuation, next syllable starts with lowercase).
        // A repeat barline is a musical structure marker, not necessarily a phrase break.
        var barTypeByTick = {};
        for (var bti = 0; bti < barlines.length; bti++) {
            barTypeByTick[barlines[bti].tick] = barlines[bti].type;
        }
        for (var si2 = 0; si2 < syllables.length; si2++) {
            if (!syllables[si2].sectionBar) continue;
            var syl = syllables[si2];
            var sEnd = syl.tick + Math.round((syl.durationQ || 1) * division);
            var bType = barTypeByTick[sEnd];
            if (bType !== "endRepeat") continue;
            // Check: text does NOT end with strong punctuation
            var text = syl.text || "";
            var lastChar = text.charAt(text.length - 1);
            if (lastChar === "." || lastChar === "!" || lastChar === "?" || lastChar === "\uFE52") continue;
            // Check: next syllable in same verse starts with lowercase
            for (var si3 = si2 + 1; si3 < syllables.length; si3++) {
                if (syllables[si3].verse === syl.verse) {
                    var nextText = syllables[si3].text || "";
                    var firstChar = nextText.charAt(0);
                    if (firstChar && firstChar === firstChar.toLowerCase() && firstChar !== firstChar.toUpperCase()) {
                        syllables[si2].sectionBar = false;
                    }
                    break;
                }
            }

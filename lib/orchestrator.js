    ...
    if (sysTexts.length > 0 && allStreamSyls.length > 0) {
        ...
        for (var dsi = 1; dsi < allStreamSyls.length; dsi++) {
            ...
            if (curr._jumpReplay && (dsi === 1 || !prev._jumpReplay)) {
                ...
                // When the gap covers the intro section and introChords are
                // available, use them (already expanded for repeats).
                for (var dgl = 0; dgl < gapLabels.length; dgl++) {
                    ...
                    var glChords;
                    if (introChords && introChords.length > 0 && glFrom <= firstSylTick) {
                    ...
                    if (glChords.length > 0 && data.fullRepeat) {

...
function processWithExpander(data, mods) {
    ...
    if (sysTexts.length > 0 && allStreamSyls.length > 0) {
        ...
        if (dsInsertions.length > 0) {
            ...
            // Find the label at gapEndTick (the section that follows the gap).
            // Only use it if the label is AT gapEndTick (same position as first
            // DS lyric). Labels far from gapEndTick belong to other sections.
            var insertLabel = "";
            ...
            if (insertLabel) {
                // Find the D.S. replay occurrence of this label in the output.
                // Extract the base name: strip # (auto-number) and sequence
                // suffixes (colon/dash sequences like "1::2::") to match the
                // rendered label which uses just the base (e.g. "ESTROFA").
                var labelUpper = insertLabel.toUpperCase();
                ...
                // Find the D.S. replay occurrence of this label.
                // Strategy: find the LAST occurrence, which corresponds to the
                // D.S. replay pass (first-pass labels come earlier in output).
                // But only if the label actually appears 2+ times. If it appears
                // only once, the gap content should append at end instead.
                var insertPos = -1;
                var searchPos = 0;
                var allPositions = [];
                while (true) {

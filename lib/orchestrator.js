...
function processExtraction(data, mods) {
    ...
    if (hasVoltas) {
        ...
        return mods.Formatter.formatPerfLines(streamLines, introChords, homeChord, title, chords, syllables, data.systemTexts, data.fullRepeat);
    }
    ...
    if (verseNums.length === 1) {
        ...
        if (data.systemTexts) {
            inlineSystemTexts = [];
            for (var stf = 0; stf < data.systemTexts.length; stf++) {
                if (data.systemTexts[stf].tick >= firstLineTick) {
                    inlineSystemTexts.push(data.systemTexts[stf]);
                }
            }
        }
        var res1v = mods.Formatter.formatLines(lines0, chords, null, -1, inlineSystemTexts);
        output1v += res1v.output;

        // Coda instrumental
        var lastEndTick1v = lines0[lines0.length - 1].endTick;
        var codaChords1v = mods.ChordUtils.getChordsInRange(chords, lastEndTick1v, -1, res1v.lastChord);
    ...
    if (verse0Repeated.length > 0) {
        ...
        var res0MV = mods.Formatter.formatLines(verse0Repeated, chords, lastChordMV, repeatEndTickMV);
        outputMV += res0MV.output;
        lastChordMV = res0MV.lastChord;
    }
    ...
    for (var v5 = 1; v5 < verseNums.length; v5++) {
        ...
        var res5MV = mods.Formatter.formatLines(vLines5, chords, lastChordMV, repeatEndTickMV);
        outputMV += res5MV.output;
        lastChordMV = res5MV.lastChord;
    }
    ...
    if (verse0Coda.length > 0) {
        ...
        outputMV += resCMV.output;
        lastChordMV = resCMV.lastChord;
    }
...
function processWithNavigation(data, mods) {
    ...
    var deferredInstrumental = []; // instrumental D.S. replay chord output
    ...
    for (var pi = 0; pi < plan.length; pi++) {
        ...
        // If this is a replay segment (overlapCount > 0) and fullRepeat is off,
        // check if there are UNCONSUMED new lyrics (verse 1+). If not, skip.
        // Verse 1 syllables may have already been consumed by repeat expansion in segment 1.
        if (overlapCount > 0 && !fullRepeat) {
            var replaySyls = mods.PerfStream.filterSylsByRange(syllables, seg.fromTick, seg.toTick);
            var hasNewVerse = false;
            for (var nv = 0; nv < replaySyls.length; nv++) {
                if (replaySyls[nv].verse > 0) {
                    var cvKey = replaySyls[nv].tick + "_" + replaySyls[nv].verse;
                    if (!consumedSyls[cvKey]) { hasNewVerse = true; break; }
                }
            }
            if (!hasNewVerse) {
                // No new lyrics, but if honorRepeats, save instrumental chord output for later
                if (seg.honorRepeats) {
                    var instRepeats = (data.repeats || []).filter(function(r) {
                        return r.startTick >= seg.fromTick && r.endTick <= seg.toTick;
                    });
                    var instVoltas = (data.voltas || []).filter(function(v) {
                        return v.startTick >= seg.fromTick && v.endTick <= seg.toTick;
                    });
                    if (instRepeats.length > 0) {
                        var instSections = mods.RepeatStructure.buildSections(instRepeats, instVoltas);
                        var lastInstChord = null;
                        var instOutput = "\n";
                        for (var isec = 0; isec < instSections.length; isec++) {
                            var irep = instSections[isec].repeat;
                            var iv1 = instSections[isec].volta1;
                            var iv2 = instSections[isec].volta2;
                            var mainEnd = iv1 ? iv1.startTick : irep.endTick;
                            for (var ipass = 0; ipass < 2; ipass++) {
                                var passChords = mods.ChordUtils.getChordsInRange(chords, irep.startTick, mainEnd, lastInstChord);
                                var passVolta = (ipass === 0 && iv1) ? iv1 : (ipass === 1 && iv2) ? iv2 : null;
                                if (passVolta) {
                                    var vChords = mods.ChordUtils.getChordsInRange(chords, passVolta.startTick, passVolta.endTick,
                                        passChords.length > 0 ? passChords[passChords.length - 1] : lastInstChord);
                                    passChords = passChords.concat(vChords);
                                }
                                if (passChords.length > 0) {
                                    instOutput += mods.Formatter.wrapChordLine(passChords.join("  "), 70) + "\n";
                                    lastInstChord = passChords[passChords.length - 1];
                                }
                            }
                        }
                        var lastRepEnd = instRepeats[instRepeats.length - 1].endTick;
                        var postRepChords = mods.ChordUtils.getChordsInRange(chords, lastRepEnd, seg.toTick, lastInstChord);
                        if (postRepChords.length > 0) {
                            instOutput += mods.Formatter.wrapChordLine(postRepChords.join("  "), 70) + "\n";
                        }
                        deferredInstrumental.push(instOutput);
                    }
                }
                continue;
            }
    ...
    var output = mods.Formatter.formatPerfLines(streamLines, introChords, homeChord, title, chords, syllables, data.systemTexts, data.fullRepeat);

    // Append deferred instrumental D.S./D.C. replay chord output
    for (var di = 0; di < deferredInstrumental.length; di++) {
        output += deferredInstrumental[di];
    }

...
// Rule printed above each orphan stanza. The verses below it are not part of the music
// the score plays, and nothing else in the output says so, hence the visual break.
var ORPHAN_RULE = new Array(25).join("- ").replace(/\s+$/, "");

// Stanzas for the verses no pass sings, printed with their chords after the last pass
// (spec 7.1.2). Only reached with the orphan lyrics option on.
function orphanVerseStanzas(data, mods, verses, chords) {
    ...
    for (var vi = 0; vi < verses.length; vi++) {
        ...
        // A verse reaches no further than its own last note. Chords after that belong to
        // whatever the score plays next, and would otherwise pile up as a trailing run.
        var lastSyl = vSyls[vSyls.length - 1];
        var verseEnd = lastSyl.tick + Math.round((lastSyl.durationQ || 1) * division);
        var vChords = (chords || []).filter(function(c) { return c.tick < verseEnd; });
        var vLines = mods.LineBuilder.mergeShortLines(
            mods.LineBuilder.splitLongLines(
                mods.LineBuilder.buildLinesFromWords(words, vChords), vSyls));
        if (vLines.length === 0) continue;
        mods.LineBuilder.applyStanzaFormatting(vLines);
        var res = mods.Formatter.formatLines(vLines, vChords, null, -1, null);
        out += "\n" + ORPHAN_RULE + "\n" + (res.output || res);
    }
    return out;
}
...
function processExtraction(data, mods) {
    ...
    if (verseNums.length === 1) {
        ...
        // "Verse #" and "Soloist a:b" templates expand here too. The counters are
...
// template ("Verse #", "Verse 1::2::3::") cannot be rendered here: the sequence
...
function processWithExpander(data, mods) {
    ...
    if (introHasRepeats) {
        ...
        // The entries keep the tick of each played chord, which is what lets the formatter cut
        // the intro at a label without going back to the written order
        introEntries = mods.IntroChords.buildIntroEntriesPerf(chords, introRepStruct, syllables, firstSylTick);
        introChords = introEntries.map(function(e) { return e.chord; });
    } else {
    ...
    var result = mods.Formatter.formatPerfLines(streamLines, introChords, homeChord, title, chords, syllables, data.systemTexts, data.fullRepeat, repStart, rawRepeats, introEntries);
    ...
    if (sysTexts.length > 0 && allStreamSyls.length > 0) {
        ...
        // the gap, e.g. "VERSE 3") and insert before it.
    ...
    // Verses no pass sings, when the orphan lyrics option asks for them (spec 7.1.2)
    if (data.orphanLyrics) {
        var orphans = orphanVerses(data, mods);
        if (orphans.length > 0) output += orphanVerseStanzas(data, mods, orphans, chords);
    }

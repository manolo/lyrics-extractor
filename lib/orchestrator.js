...
function processExtraction(data, mods) {
    ...
    if (hasVoltas) {
        ...
        var repStart = data.repeats && data.repeats.length > 0 ? data.repeats[0].startTick : -1;
        var result = mods.Formatter.formatPerfLines(streamLines, introChords, homeChord, title, chords, syllables, data.systemTexts, data.fullRepeat, repStart);
...
function processWithNavigation(data, mods) {
    ...
    var repStart2 = data.repeats && data.repeats.length > 0 ? data.repeats[0].startTick : -1;
    var result2 = mods.Formatter.formatPerfLines(streamLines, introChords, homeChord, title, chords, syllables, data.systemTexts, data.fullRepeat, repStart2);

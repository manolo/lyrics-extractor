...
// Chords, and the annotations that share their line. Courier advances 0.6 em per character,
// which is what lets a run be placed by counting columns rather than measuring text.
var CHORD_RGB = "0.298 0.686 0.314";
var ANNOTATION_RGB = "0.85 0.45 0.10";
var COURIER_WIDTH = 0.6;

// Split a chord line into runs to be drawn in one colour each. A braced run is an annotation,
// which the extractor wrapped so that everything reading this line back can tell words from a
// harmony. Here the colour says it instead, so each brace is drawn as a space rather than
// dropped: taking the two characters out would pull the rest of the line two columns left and
// the chords would no longer sit over their syllables.
function splitAnnotations(text) {
    ...
    while (i < text.length) {
        ...
        if (open < 0) {
            runs.push({ col: i, text: text.substring(i), isText: false });
            break;
        }
        ...
        if (open > i) runs.push({ col: i, text: text.substring(i, open), isText: false });
        runs.push({ col: open, text: " " + text.substring(open + 1, close) + " ", isText: true });
        i = close + 1;
    }
    return runs;
}
...
function generatePdf(textOutput, options) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        if (options.lineNumbers && !isChord) {
            ...
            var chordText = stripMarker(line);
            var runs = splitAnnotations(chordText);
            for (var r = 0; r < runs.length; r++) {
                currentPage.push({
                    type: "text",
                    x: MARGIN_L + runs[r].col * scaledFontChord * COURIER_WIDTH,
                    y: y,
                    font: "Courier", size: scaledFontChord,
                    color: runs[r].isText ? ANNOTATION_RGB : CHORD_RGB,
                    text: runs[r].text
                });
            }
        } else {
            currentPage.push({
                type: "text", x: MARGIN_L, y: y,
                font: "Courier", size: scaledFontLyric,
                color: "0 0 0",
                text: line
            });
        }
...
if (typeof exports !== "undefined") {
    ...
    exports.splitAnnotations = splitAnnotations;

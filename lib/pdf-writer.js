...
function renderFretDiagram(diagram, x, y) {
    ...
    // Keep fret spacing constant so all frets have the same height
    ...
    if (diagram.barre) {
        // Fret numbers in XML are already relative to the diagram (1-N), don't subtract fretOffset
        ...
        if (fretNum >= 1 && fretNum <= fretsToRender) {
            ...
            // Handle barre.end = -1 (means last string, which is index 5)
        ...
        if (firstNonEmpty) {
            ...
            if (titleText === titleText.toUpperCase() && titleText.length > 0 && !isChordLine(titleText)) {
                ...
                if (!diagramsRendered && options.fretDiagrams && options.fretDiagrams.length > 0) {
                    ...
                    var diagramSpacing = 28; // Horizontal spacing between diagrams

...
function generatePdf(textOutput, options) {
    ...
    // Group footer (centered at bottom of last page, with separator line above)
    if (options.header) {
        var footerY = 10;
        var lineY = footerY + scaledFontGroup + 4;
        currentPage.push({
            type: "line",
            x1: MARGIN_L, y1: lineY,
            x2: PAGE_W - MARGIN_R, y2: lineY,
            width: 0.5
        });
        currentPage.push({

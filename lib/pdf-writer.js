...
// diagram: {chordName, strings: [{number, marker?, dot?}], fretOffset}
// x, y: top-left corner position
function renderFretDiagram(diagram, x, y) {
    var DIAGRAM_W = 21.3;
    var STRING_SPACING = 4.5;
    var GRID_W = 5 * STRING_SPACING; // Width spans exactly 5 string spaces
    var GRID_H = 32;
    var FRET_SPACING = 8;
    var LABEL_SIZE = 7;
    var DOT_RADIUS = 1.5;
    ...
    // 1. Chord name (centered above grid)
    stream += "BT\n/F2 " + LABEL_SIZE + " Tf\n0 0 0 rg\n";
    var labelW = diagram.chordName.length * LABEL_SIZE * 0.5;
    var labelX = x + (DIAGRAM_W - labelW) / 2;
    stream += labelX.toFixed(2) + " " + y.toFixed(2) + " Td\n";
    stream += "(" + escPdfString(diagram.chordName) + ") Tj\nET\n";
    
    // 2. Fretboard grid (6 vertical strings, 5 horizontal fret lines)
    stream += "0.3 w\n0 0 0 RG\n";
    var NUT_WIDTH = diagram.fretOffset === 0 ? 2 : 0.3; // Nut line width
    for (var s = 0; s < 6; s++) {
        var sx = gridX + s * STRING_SPACING;
        // Extend strings to the top edge of the nut line (half the nut width up)
    ...
    for (var f = 0; f <= 4; f++) {
        ...
        var lw = (f === 0 && diagram.fretOffset === 0) ? 2 : 0.3;
        stream += lw + " w\n";
    ...
    if (diagram.fretOffset > 0) {
        ...
        stream += "(" + diagram.fretOffset + ") Tj\nET\n";
    }
    
    // 4. String markers and dots
    for (var i = 0; i < diagram.strings.length; i++) {
        ...
        if (str.marker === "cross") {
            ...
            var fretNum = str.dot.fret - diagram.fretOffset;
            if (fretNum >= 1 && fretNum <= 4) {
                var dotY = gridY - fretNum * FRET_SPACING + FRET_SPACING / 2;
                stream += "0 0 0 rg\n";
...
// options: { header: string, onePage: boolean }
function generatePdf(textOutput, options) {
    ...
    var pages = [];
    var currentPage = [];
    var y = PAGE_H - MARGIN_T - diagramHeaderHeight;

    function newPage() {
        if (currentPage.length > 0) pages.push(currentPage);
        currentPage = [];
        y = PAGE_H - MARGIN_T - diagramHeaderHeight;
    }
    ...
    // Render fretboard diagrams header (after header, before lyrics)
    if (options.fretDiagrams && options.fretDiagrams.length > 0) {
        var diagramY = PAGE_H - MARGIN_T - 5;
        var diagramX = MARGIN_L;
        var diagramSpacing = 27;

...
// Chord lines also carry text annotations (staff text, expressions), wrapped in braces by the
// extractor. ChordPro reads every [...] as a transposable chord, so annotations use the
// text-annotation form [*text] instead, and the braces come off: the star is the marker here.
// An annotation is words, and keeps whatever it says.
...
function parseChords(chordLine) {
    ...
    while (i < chordLine.length) {
        if (chordLine[i] !== " ") {
            var start = i;
            // A braced annotation is one token even though it has spaces in it: the braces
            // are what hold "{Continue rhythm}" together where whitespace cannot.
            if (chordLine[i] === "{") {
                while (i < chordLine.length && chordLine[i] !== "}") i++;
                if (i < chordLine.length) i++;
            } else {
                while (i < chordLine.length && chordLine[i] !== " ") i++;
            }

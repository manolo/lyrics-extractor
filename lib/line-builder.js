...
// Convert ... (3+) to ellipsis (U+2026), .. (exactly 2) to small full stop (U+FE52).
// Convert ,, to small comma (U+FE50).
...
function findPosForTick(sylMap, tick) {
    ...
    if (bestIdx >= 0 && bestIdx + 1 < sylMap.length) {
        ...
        if (tick > midpoint) {
            var nextPos = sylMap[bestIdx + 1].pos;
            // Place at the space before the next syllable if there is one
            bestPos = (nextPos > 0) ? nextPos - 1 : nextPos;
        }
    }
    return bestPos;
}

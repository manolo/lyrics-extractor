...
// Convert consecutive dots (2+) to ellipsis character (U+2026).
// Only replace a SINGLE dot between letters (synalepha like "da.es").
function cleanWordText(text) {
    // First: convert consecutive dots (2+) to ellipsis unicode
...
function buildLinesFromWords(words, chords) {
    ...
    if (currentLineText.trim()) {
        lines.push({
            ...
            sylMap: currentSylMap,
            startTick: currentLineStartTick,
            endTick: words[words.length - 1].tick,
            sectionEnd: words[words.length - 1].sectionEnd || false
        });
    }
...
function applyStanzaFormatting(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        // Convert semicolons and fullwidth commas to regular commas in output

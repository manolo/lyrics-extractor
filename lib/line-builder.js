...
// syllables: optional expanded syllables array for rest-based splitting
function splitLongLines(lines, syllables) {
    // Absolute maximum: no line should exceed this if it has split points
    var MAX_LINE = 70;

    // Calculate median line length in characters (needs 3+ lines)
    var lengths = [];
    for (var i = 0; i < lines.length; i++) {
        if (lines[i].text) lengths.push(lines[i].text.length);
    }

    var median, threshold;
    if (lengths.length >= 3) {
        lengths.sort(function(a, b) { return a - b; });
        median = lengths[Math.floor(lengths.length / 2)];
        threshold = Math.floor(median * 1.3);
        if (threshold > MAX_LINE) {
            threshold = MAX_LINE;
            median = Math.floor(MAX_LINE / 2);
        }
        if (median < 10) return lines;
    } else {
        // Too few lines for median: use absolute max only
        threshold = MAX_LINE;
        median = Math.floor(MAX_LINE / 2);
    }
    ...
    while (changed) {
        ...
        for (var li = 0; li < lines.length; li++) {
            ...
            // Build rest positions from syllables for this line's tick range
            var restPositions = [];
            if (syllables) {
                for (var rsi = 0; rsi < syllables.length; rsi++) {
                    var rs = syllables[rsi];
                    if (rs.tick < line.startTick || rs.tick > line.endTick) continue;
                    var minRestQ = line.text.length > 75 ? 0.25 : 2;
                    if (!rs.restAfter || rs.restDurationQ < minRestQ) continue;
                    if (rs.syllabic !== "end" && rs.syllabic !== "single") continue;
                    // Find the text position of this syllable's tick in the sylMap
                    for (var rmi = 0; rmi < line.sylMap.length; rmi++) {
                        if (line.sylMap[rmi].tick === rs.tick) {
                            // Position AFTER this word (find next space)
                            var afterPos = line.sylMap[rmi].pos;
                            var spaceAfter = line.text.indexOf(" ", afterPos);
                            if (spaceAfter > 0) {
                                restPositions.push({ pos: spaceAfter, restQ: rs.restDurationQ });
                            }
                            break;
                        }
                    }
            ...
            var split = findBestSplit(line.text, median, restPositions);
            if (!split) {
                result.push(line);
                continue;
            }
            ...
            // Lines split by overlong splitting (space or rest) are forced breaks
            // that should not get a comma added by applyStanzaFormatting.
            // Only comma splits correspond to actual phrase boundaries.
            if (split.type === "space" || split.type === "rest") line1._noComma = true;
            result.push(line1);
            result.push({
                text: text2,
                sylMap: sylMap2,
...
// restPositions: optional array of {pos, restQ} for rest-based splitting.
// Returns { pos, text1, text2, type } or null if no good split found.
function findBestSplit(text, targetLen, restPositions) {
    var bestComma = null;
    ...
    // Fallback: split at rest boundaries (word boundary after a musical rest)
    if (restPositions && restPositions.length > 0) {
        var bestRest = null;
        var bestRestScore = -1;
        for (var ri = 0; ri < restPositions.length; ri++) {
            var rp = restPositions[ri];
            if (rp.pos < minHalf || (text.length - rp.pos) < minHalf) continue;
            // Score: prefer higher rest, then closer to target
            var rDist = Math.abs(rp.pos - targetLen);
            var score = rp.restQ * 1000 - rDist;
        ...
        if (bestRest !== null) {
            // Find the space at or after the rest position to split cleanly
            var splitAt = bestRest.pos;
            // splitAt should point to a space between words
            if (text[splitAt] !== ' ') {
                var nextSpace = text.indexOf(' ', splitAt);
                if (nextSpace > 0 && nextSpace < text.length - minHalf) splitAt = nextSpace;
            }
            if (splitAt > 0 && splitAt < text.length) {
                return {
                    pos: splitAt + 1,
                    text1: text.substring(0, splitAt).replace(/\s+$/, ""),
                    text2: text.substring(splitAt + 1).replace(/^\s+/, ""),
                    type: "rest"
                };
            }
        }
    ...
    // Last resort for very long lines: split at the space nearest to midpoint
    if (text.length > 90) {
        var mid = Math.floor(text.length / 2);
        var bestSpace = -1;
        var bestSpaceDist = text.length;
        for (var si = 1; si < text.length - 1; si++) {
            if (text[si] === ' ') {
                var sDist = Math.abs(si - mid);
                if (sDist < bestSpaceDist && si >= minHalf && (text.length - si) >= minHalf) {
                    bestSpaceDist = sDist;
                    bestSpace = si;
                }
            }
        ...
        if (bestSpace > 0) {
            return {
                pos: bestSpace + 1,
                text1: text.substring(0, bestSpace),
                text2: text.substring(bestSpace + 1),
                type: "space"
            };
        }
    }
...
function applyStanzaFormatting(lines) {
    ...
    for (var i = 0; i < lines.length; i++) {
        ...
        // but only if the word is not intentionally uppercase (e.g. "CAMPFIRE")
        if (i > 0) {
            var prevText = lines[i - 1].text;
            var prevLast = prevText ? prevText.charAt(prevText.length - 1) : '';
            if (prevLast === ',') {
                var firstWord = text.split(/\s/)[0];
                var isAllCaps = firstWord.length > 1 && firstWord === firstWord.toUpperCase();
                if (!isAllCaps) {
        ...
        if (i < lines.length - 1) {
            ...
            // Skip comma for lines that were force-split at a plain space
            if (!hasPunctuation && !lines[i]._noComma) {
                text += ',';
            }
        } else {
    ...
    while (changed) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (words.length <= 4 && !hasPunctuation && !line.sectionEnd && i + 1 < lines.length) {
                ...
                // Don't merge if the result would exceed the PDF printable width
                var mergedLength = text.length + 1 + nextText.length;
                if ((!nextIsCapital || skipCapitalCheck) && nextText && mergedLength <= 80) {

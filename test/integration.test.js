...
//
// The corpus in test/its/scores/ travels with the repository, with its baselines in
// test/its/baselines/. Each score exists to reach code the others do not: no lyrics at all,
// repeats that carry no lyrics, phrases that have to be split, chord spellings, section labels,
// instrumental intros, chord diagrams in the guitar part. test/its/snapshot.js drives them,
// discovering what to run from the baselines present.
//
// A developer who tests against real scores keeps them, their baselines and a suite of their own
// in test/local/, which git ignores and "npm test" picks up when it is there. Nothing about that
// music appears here.
...
snapshot.define({
    baselinesDir: path.join(ITS_DIR, "baselines"),
    scoresDir: path.join(ITS_DIR, "scores"),
    cli: CLI,
    // The score is committed, so its mtime is whenever the checkout happened and comparing it
    // against the marker in the baseline would mean nothing
    note: function() {
        return "NOTE: this score is committed with the repository, so its mtime says nothing." +
            "\n  Either the output changed for a real reason, or the score itself was replaced.";
    }
});
